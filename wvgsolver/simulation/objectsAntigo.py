...
class Waveguide(SimulationObject):
  ...
  def __init__(self, structures=[], size=Vec3(1e-6), engine=None, load_path=None, metadata=None,
    ...
    self._structures = structures if isinstance(structures, list) else [structures] 
    self._size = size
    super().__init__(engine=engine, load_path=load_path, metadata=metadata,**kwargs)
...
class Cavity1D(Waveguide):
  ...
  def __init__(self, unit_cells=[], structures=[], size=None, center_cell=None,
               ...
               component = 'Ey', boundary = 'ymin', boundarySymmetry = 'antisymmetric',
               **kwargs):
    """
    Parameters
    ----------
    ...
    self.boundarySymmetry = boundarySymmetry
    ...
    super().__init__(structures=structures, engine=engine,
                     load_path=load_path, size=size, metadata=metadata,
                     **kwargs)

    self._default_sim_type = "resonance"
    self._no_sess_sims = ["quasipotential",'mpb']
    self._unit_cellsC = []
  ...
  def _simulate_modeProfile(self,sess,wl,fwhm, \
      simTime=200e-15,  sim_size=Vec3(2, 4, 4),**kwargs):
    ...
    boundaries = self.boundary.split()
    symmetries = self.boundarySymmetry.split()
    boundariesDict = {boundaries[i]:symmetries[i] for i in range(len(boundaries))}

##    sess.set_sim_region(size=bbox.size * Vec3(1.2, 1, 1), boundaries=boundariesDict)
    ...
    sess.set_sources(DipoleSource(frange = (299792458/(wl+fwhm/2),299792458/(wl-fwhm/2)),
                                  pulse_offset=2.1*6e-15,
                                  pos=Vec3(0), axis=axis,dipole_type = dipole_type))

    
    sess.set_sim_time(simTime)

    target_freq = 299792458/wl
    st = 180e-15
    analysis = {
  ...
  def _simulate_resonance(self, sess, centerWl = 1e-6, deltaWl = 0.1e-6,target_freq=None, source_pulselength=60e-15, analyze_fspan=3e14, \
      analyze_time=590e-15, eref_time=80e-15, TEonly=True, sim_size=Vec3(2, 4, 4), energy_downsample=2):
    """Simulate the cavity's resonaar thnce frequency and obtain directional Q factors, mode volume, and 
...
##    sess.set_sim_region(size=bbox.size * Vec3(1.2, 1, 1), boundaries=boundariesDict)
    ...
    dipole_type = 'Magnetic dipole' if self.component[0] == 'H' else 'Electric dipole'
    print('vecMultiplier',vecMultiplier)
    sess.set_sim_region(size=bbox.size * vecMultiplier, boundaries=boundariesDict)
    ...
    print('freqs',spectrum_freqs.min(),spectrum_freqs.max())
    ...
    analysis = {
      "res": FrequencySpectrum(BBox(bbox.pos, size*bboxMultiplier), spectrum_freqs,
                               name = 'res',start_time = startTime,nmonitors=5),
      "resLumerical": QLumerical(BBox(bbox.pos, size*bboxMultiplier), spectrum_freqs,
                               name = 'resLum',start_time = startTime,nmonitors=5),
...
##      "xyprofile": WaveProfile(BBox(bbox.pos + Vec3(0, 0, 0.45*size.z), bbox.size), AXIS_Z, st, target_freq),
      "xyprofile": WaveProfile(BBox(bbox.pos + Vec3(0, 0, 0), bbox.size), AXIS_Z, st, target_freq),
      "yzprofile": WaveProfile(bbox, AXIS_X, st, target_freq),
      'harminv':Harminv(BBox(bbox.pos, size*bboxMultiplier), spectrum_freqs,self.component,start_time = startTime,
                        name = 'harminv',
                        pos = Vec3(0,0.123e-6,0) if self.direction == 'y' else Vec3(0.123e-6,0,0)),
    }


    t = sess.fdtd.addtime(name='teste', monitor_type=2, y=0,
            z=0, output_Hx=False, output_Hy=False, output_Hz=False, min_sampling_per_cycle=10000,
                              start_time = startTime,
                      x_min = 0.1e-6,x_max = 0.4e-6)
    ...
    res,signal,harminvOut,harminvErr,harminvField,harminvTime  = sess.analyze("res")
    freqLum,Qlum,ldaLum = sess.analyze('resLumerical')
    freq = spectrum_freqs[np.argmax(res)]
    harminvOut2,harminvErr2,harminvField2,harminvTime2 = sess.analyze('harminv')

    print('res ok')
    
##    E2 = f.getelectric(tm);
##    bm = E2 < 0.5*E2;E2[bm] = 0;
##    Integral = trapz(trapz(trapz(E2,xv,axis=0),yv,axis=0),zv,axis=0)
...
##    eref,_,_,_,_ = sess.analyze("eref", freq = freq)
    ...
    self.savedResults = {'e':e,
                         'pxmax':sess.analyze("pxmax"),
                         'pymax':sess.analyze("pymax"),
                         'pzmax':sess.analyze("pzmax"),
                         'edensity':edensity,
                         'res':res,
                         'spectrum_freqs':spectrum_freqs,
                         'signal':signal,
                         'harminvField':harminvField,
                         'harminvTime':harminvTime,
                         'harminvOut':harminvOut,
                         'harminvField2':harminvField2,
                         'harminvTime2':harminvTime2,
                         'harminvOut2':harminvOut2}

    qxmax = 2*np.pi*freq*e/self.savedResults["pxmax"]      
    qymax = 2*np.pi*freq*e/self.savedResults["pymax"]
    qzmax = 2*np.pi*freq*e/self.savedResults["pzmax"]
    ...
    try:
      ...
      harminvOut = pd.read_csv(StringIO(harminvOut))
      harminvOut = harminvOut.rename(columns = lambda x: x.strip())
      harminvOut['frequency'] *= 10**15
      harminvOut['wl'] = 299792458.0/harminvOut['frequency']
##      bm = (harminvOut['wl'] > sess.fdtd.c()/freq2) & (harminvOut['wl'] < sess.fdtd.c()/freq1) & (harminvOut['Q'] > 0)
      bm = (harminvOut['Q'] > 0)
      harminvOut = harminvOut[bm]


      from io import StringIO
      harminvOut2 = pd.read_csv(StringIO(harminvOut2))
      harminvOut2 = harminvOut2.rename(columns = lambda x: x.strip())
      harminvOut2['frequency'] *= 10**15
      harminvOut2['wl'] = 299792458.0/harminvOut2['frequency']
##      bm = (harminvOut['wl'] > sess.fdtd.c()/freq2) & (harminvOut['wl'] < sess.fdtd.c()/freq1) & (harminvOut['Q'] > 0)
      bm = (harminvOut2['Q'] > 0)
      harminvOut2 = harminvOut2[bm]
    ...
    if 'ymin' not in boundaries:
      self.savedResults['pymin'] = sess.analyze('pymin')
      qymin = 2*np.pi*freq*e/self.savedResults["pymin"]
      output["qymin"] = qymin
    else:
      output["qymin"] = qymax

    if 'xmin' not in boundaries:
      self.savedResults['pxmin'] = sess.analyze('pxmin')
      qxmin = 2*np.pi*freq*e/self.savedResults["pxmin"]
      output["qxmin"] = qxmin
    else:
      output["qxmin"] = qxmax
      
    if 'zmin' not in boundaries:
      self.savedResults['pzmin'] = sess.analyze('pzmin')
      qzmin = 2*np.pi*freq*e/self.savedResults["pzmin"]
      output["qzmin"] = qzmin
    else:
      output["qzmin"] = qzmax
...
class Crossbeam(Cavity1D):
    ...
    def _simulate_resonance2(self, sess, target_freq=400e12, source_pulselength=60e-15, analyze_fspan=3e14, 
        analyze_time=590e-15, eref_time=80e-15, TEonly=True, sim_size=Vec3(2, 4, 4), energy_downsample=2):
      """Simulate the cavity's resonaar thnce frequency and obtain directional Q factors, mode volume, and 
      electric field profile data. The simulation comprises of one dipole polarized along the y-axis
      in the center of the cavity, and all simulation results are calculated in the last 20 fs 
  ...
  ##    sess.set_sim_region(size=bbox.size * Vec3(1.2, 1, 1), boundaries=boundariesDict)
      ...
      if self.direction == 'x':
        ...
        bboxMultiplier = Vec3(0,0.3,0)
  ##    if len(self._unit_cells) > 0:
  ##      bboxMultiplier.x = 0.3
  ##    if len(self._unit_cellsC) > 0:
  ##      bboxMultiplier.y = 0.3



      analysis = {
        "res": FrequencySpectrum(BBox(bbox.pos, size*bboxMultiplier), spectrum_freqs),
        'resEx': FrequencySpectrum(BBox(bbox.pos, size*Vec3(0,0.3,0)), spectrum_freqs, component = 'Ex'),
        'resEy': FrequencySpectrum(BBox(bbox.pos, size*Vec3(0.3,0,0)), spectrum_freqs, component = 'Ey'),
  ##      "eref": WaveEnergy(bbox, target_freq, eref_time, energy_downsample),
        "e": WaveEnergy(bbox, target_freq, st, energy_downsample),
        "pxmax": SideWavePower(bbox, AXIS_X, 1, st, target_freq),
        "pymax": SideWavePower(bbox, AXIS_Y, 1, st, target_freq),
  ...
  ##      "xyprofile": WaveProfile(BBox(bbox.pos + Vec3(0, 0, 0.45*size.z), bbox.size), AXIS_Z, st, target_freq),
        "xyprofile": WaveProfile(BBox(bbox.pos + Vec3(0, 0, 0), bbox.size), AXIS_Z, st, target_freq),
        "yzprofile": WaveProfile(bbox, AXIS_X, st, target_freq),
        'harminv':Harminv(BBox(bbox.pos, size*bboxMultiplier), spectrum_freqs,self.component,
                          name = 'Harminv'),
        'harminvEx':Harminv(BBox(bbox.pos, size*Vec3(0,1,0)), spectrum_freqs,component = 'Ex',
                            name = 'harminvEx', pos = Vec3(0,0.423e-6,0)),
        'harminvEy':Harminv(BBox(bbox.pos, size*Vec3(1,0,0)), spectrum_freqs,component = 'Ey',
                            name = 'harminvEy', pos = Vec3(0.423e-6,0,0))
      }
      ...
      resX = sess.analyze("resEx")
      resY = sess.analyze("resEy")
      freq = spectrum_freqs[np.argmax(res)]
      freqX = spectrum_freqs[np.argmax(resX)]
      freqY = spectrum_freqs[np.argmax(resY)]
      harminvOut,harminvErr = sess.analyze('harminv')
      harminvOutEx,harminvErrEx = sess.analyze('harminvEx')
      harminvOutEy,harminvErrEy = sess.analyze('harminvEy')
  ...
  ##    eref,_,_,_,_ = sess.analyze("eref", freq = freq)
      ...
      try:
        ...
        harminvOutEx = pd.read_csv(StringIO(harminvOutEx))
        harminvOutEx = harminvOutEx.rename(columns = lambda x: x.strip())
        harminvOutEx['frequency'] *= 10**15
        harminvOutEx['wl'] = 299792458.0/harminvOutEx['frequency']


        harminvOutEy = pd.read_csv(StringIO(harminvOutEy))
        harminvOutEy = harminvOutEy.rename(columns = lambda x: x.strip())
        harminvOutEy['frequency'] *= 10**15
        harminvOutEy['wl'] = 299792458.0/harminvOutEy['frequency']
      ...
      output = {
        ...
        'harminvOut':harminvOut,
        'harminvErr':harminvErr,
        'harminvOutEx':harminvOutEx,
        'harminvErrEx':harminvErrEx,
        'harminvOutEy':harminvOutEy,
        'harminvErrEy':harminvErrEy,
        'freqEx':freqX,
        'freqEy':freqY,
        'wlEx':(10**9)*C_LIGHT/freqX,
        'wlEy':(10**9)*C_LIGHT/freqY
      }

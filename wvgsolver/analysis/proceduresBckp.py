...
##    f = sess.fdtd
##    
##    Ex = f.pinch(f.getdata(mnames[0],'Ex'))
##    Ey = f.pinch(f.getdata(mnames[0],'Ey'))
##    Ez = f.pinch(f.getdata(mnames[0],'Ez'))
##    t = f.pinch(f.getdata(m, 't'))
##    msignal = Ey * f.exp(-0.5*(t-max(t)*0.5)**2/((max(t)*0.25)**2))
##    fd = abs(f.czt(msignal,t,2*pi*self.freqs))

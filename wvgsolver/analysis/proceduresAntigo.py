...
class FrequencySpectrum(Analysis):
  def __init__(self, bbox, freqs, nmonitors=5, start_time=0,
               component = None, name = None, pos = None):
    ...
    for i in range(self.nmonitors):
      if name is None:
        ...
        self.monitor_names.append(name+str(i))
    self.bbox = bbox
    self.freqs = freqs
  ...
  def _setup_lumerical(self, sess):
    ...
    #change here to only positioning the monitors at the upper, right side of the simulation,
      #so it is possible to exploit x,y,z symmetry
    
    for i in range(len(self.monitor_names)):
      if self.pos is None:
        r = np.random.random_sample((3, )) * 0.5 #former was - 0.5
      ...
      if self.component == 'Ey':
        sess.fdtd.addtime(name=self.monitor_names[i], monitor_type=1, x=pos.x, y=pos.y,
        z=pos.z, output_Hx=False, output_Hy=False, output_Hz=False,
        output_Ex=False, output_Ey=True, output_Ez=False,
        min_sampling_per_cycle=10000,start_time = self.start_time)
      if self.component == 'Ex':
        sess.fdtd.addtime(name=self.monitor_names[i], monitor_type=1, x=pos.x, y=pos.y,
        z=pos.z, output_Hx=False, output_Hy=False, output_Hz=False,
        output_Ex=True, output_Ey=False, output_Ez=False,
        min_sampling_per_cycle=10000,start_time = self.start_time)
      if self.component == 'Hz':
        sess.fdtd.addtime(name=self.monitor_names[i], monitor_type=1, x=pos.x, y=pos.y,
        z=pos.z, output_Hx=False, output_Hy=False, output_Hz=True,
        output_Ex=False, output_Ey=False, output_Ez=False,
        min_sampling_per_cycle=10000,start_time = self.start_time)
  ...
  def _analyze_lumerical(self, sess):
    ...
    f = sess.fdtd
    time = t.flatten()
    field = signal.flatten()
    fieldString = ' '.join([str(i) for i in field])
    tempFile = '__fieldData%s__.dat'%self.component
    arq = open(tempFile,'w')
    arq.write(fieldString)
    arq.close()
    t = np.diff(time)[0]*10**15
    freq1 = min(self.freqs)*10**-15
    freq2 = max(self.freqs)*10**-15
    print('bash -c "source activate pmp;harminv -t %s %s-%s < %s"'%(t,freq1,freq2,tempFile))
    import subprocess
    s = subprocess.run('bash -c "source activate pmp;harminv -t %s %s-%s < %s"'%(t,freq1,freq2,tempFile),shell=True,
                       stdout = subprocess.PIPE,stderr=subprocess.PIPE)

    import os
    os.remove(tempFile)

    return fd,signal,s.stdout.decode(),s.stderr.decode(),field,time
...
class QLumerical(FrequencySpectrum):
  ...
  def __init__(self, bbox, freqs,component = None, start_time=0,**kwargs ):
...
class Harminv(FrequencySpectrum):
  '''Not fully implemented'''
  def __init__(self, bbox, freqs,component, start_time=0,**kwargs ):
    super().__init__(bbox = bbox, freqs = freqs, start_time = start_time,
                     nmonitors = 1,component = component,**kwargs)
    self.component = component

  def _analyze_lumerical(self, sess):
##    f = sess.fdtd
##    time = f.getdata(self.monitor_names[0],'t').flatten()
##    field = f.getdata(self.monitor_names[0],self.component).flatten()
##    
    f = sess.fdtd;time = f.getdata('teste','t').flatten();field = f.getdata('teste','Ey')[:,0,0,:].mean(axis=0)
    fieldString = ' '.join([str(i) for i in field])
    file = randstring
    tempFile = '__fieldData%s__.dat'%self.component
    arq = open(tempFile,'w')
    ...
    return s.stdout.decode(),s.stderr.decode(),field,time
...
class WaveProfile(Analysis):
  ...
  def _analyze_lumerical(self, sess):
    ...
    index_x = f.pinch(f.getdata(im, "index_x"))
    index_y = f.pinch(f.getdata(im, "index_y"))
    index_z = f.pinch(f.getdata(im, "index_z"))
    Ex = f.pinch(f.getdata(tm, "Ex"))
    Ey = f.pinch(f.getdata(tm, "Ey"))
    Ez = f.pinch(f.getdata(tm, "Ez"))
    d1 = f.pinch(f.getdata(tm, d1))
    d2 = f.pinch(f.getdata(tm, d2))
    t = f.pinch(f.getdata(tm, "t"))

    maxTime = np.argmax((Ex + Ey + Ez).sum(axis = (0,1)))

    return Ex[:,:,maxTime], Ey[:,:,maxTime], \
      Ez[:,:,maxTime], d1[:,0], \
      d2[:,0], np.real(index_x)
